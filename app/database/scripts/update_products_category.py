...
async def update_products_category_from_items(db: AsyncSession, dry_run: bool = True) -> dict:
    ...
    # CHỈ lấy từ items.category (không dùng products.main_category vì đang cần update)
    result = await db.execute(
        text("""
            SELECT 
                ...
                COUNT(*) as item_count
            FROM products p
            INNER JOIN items i ON p.parent_asin = i.parent_asin
    ...
    for parent_asin, data in product_categories.items():
        ...
        # Update nếu:
        # - Category khác nhau
        # - Hoặc old_category là "All Beauty" hoặc NULL
        if old_cat != new_cat or old_cat == "All Beauty" or old_cat is None or old_cat == "":
            updated_count += 1
        else:
            unchanged_count += 1
    ...
    for parent_asin, data in product_categories.items():
        ...
        # Update nếu:
        # - Category khác nhau
        # - Hoặc old_category là "All Beauty" hoặc NULL hoặc empty
        if old_cat != new_cat or old_cat == "All Beauty" or old_cat is None or old_cat == "":
            await db.execute(
                text("""
                    UPDATE products
...
async def drop_products_without_category(
    ...
    # Lấy danh sách asins từ items trước
    result = await db.execute(
        text("""
            SELECT asin
            FROM items
    ...
    item_asins = [row.asin for row in result.fetchall()]
    ...
    if item_asins:
        ...
        reviews_count = 0
        items_dropped = 0
        print(f"  Không có items nào để xóa")
