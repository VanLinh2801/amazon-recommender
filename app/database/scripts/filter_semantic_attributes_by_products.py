"""
Filter Semantic Attributes by Products in Database
==================================================
Script để filter semantic_attributes.parquet, chỉ giữ lại các products còn lại trong database.

Logic:
1. Lấy danh sách parent_asin còn lại trong database (từ bảng products)
2. Filter semantic_attributes.parquet để chỉ giữ lại các products có parent_asin trong danh sách đó
3. Backup file cũ và lưu file mới

Chạy: python app/database/scripts/filter_semantic_attributes_by_products.py
"""
...
def filter_semantic_attributes(
    project_root: Path,
    products_in_db: set[str],
    dry_run: bool = True
) -> dict:
    """
    Filter semantic_attributes.parquet để chỉ giữ lại products còn lại trong database.
    ...
    Returns:
        Dict với thống kê
    """
    semantic_attributes_file = project_root / "data" / "embedding" / "semantic_attributes.parquet"
    
    if not semantic_attributes_file.exists():
        print("⚠️  Không tìm thấy semantic_attributes.parquet, skip filter")
        return {"filtered": 0, "kept": 0, "removed": 0}
    
    print("\n" + "=" * 80)
    print("FILTER SEMANTIC_ATTRIBUTES.PARQUET BY PRODUCTS IN DATABASE")
    print("=" * 80)
    
    # Load semantic_attributes.parquet
    print(f"Đang load semantic_attributes từ {semantic_attributes_file}...")
    df = pl.read_parquet(str(semantic_attributes_file))
    ...
    print(f"\nĐang filter semantic_attributes...")
    ...
    if dry_run:
        print("\n[DRY RUN] Không thực sự filter semantic_attributes. Để filter thật, chạy với --execute")
        return {
            "filtered": 0,
    ...
    backup_dir = semantic_attributes_file.parent / "backup"
    backup_dir.mkdir(exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    
    backup_file = backup_dir / f"semantic_attributes_backup_{timestamp}.parquet"
    
    print(f"\nĐang backup file cũ...")
    shutil.copy2(semantic_attributes_file, backup_file)
    print(f"  Backup: {backup_file}")
    
    # Lưu filtered semantic_attributes
    print(f"\nĐang lưu filtered semantic_attributes...")
    df_filtered.write_parquet(str(semantic_attributes_file))
    print(f"  [OK] Đã lưu {len(df_filtered):,} items vào {semantic_attributes_file}")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Filter semantic_attributes.parquet theo products còn lại trong database")
    parser.add_argument(
        "--execute",
        action="store_true",
    ...
    print("=" * 80)
    print("FILTER SEMANTIC_ATTRIBUTES BY PRODUCTS IN DATABASE")
    print("=" * 80)
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            # Filter semantic_attributes
            stats = filter_semantic_attributes(
